...
`;
      const chunks = parseTypeScriptCode(code, "test.ts");
...
`;
  ...
  describe("fallback for files with no TS constructs", () => {
    test("returns single file chunk for files without TS constructs", () => {
      // A TS file with only imports/comments - no exportable constructs
...
`;
      ...
      // Should return a single file chunk when no TS constructs found
      expect(chunks.length).toBe(1);
...
`;
      const chunks = parseTypeScriptCode(code, "Button.tsx");
